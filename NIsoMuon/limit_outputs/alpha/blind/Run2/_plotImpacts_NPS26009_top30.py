...
plot.ModTDRStyle(l=args.left_margin, b=0.10, width=(1000 if args.checkboxes else 860), height=args.height)
...
for page in range(n):
    ...
    pads[1].SetRightMargin(max(float(pads[1].GetRightMargin()), 0.05))
    pads[0].SetGrid(1, 0)
    pads[0].SetTickx(1)
    pads[1].SetGrid(1, 0)
    pads[1].SetTickx(1)
    ...
    ROOT.TGaxis.SetExponentOffset(0.0, 0.0, "x")
    h_impacts.Draw()
